...
def distribution_two(name, param='familiar_ratio', scope=[0,4], my_dataset_name='血友病数据集', xlabel=None):
    ...
    smooth(centers_MCTS, y_MCTS, 'MCTS')
    smooth(centers_CSS_Wiki, y_CSS_Wiki, 'Wiki')
    smooth(centers_CSS, y_CSS, 'CSS')
    smooth(centers_my, y_my, 'my')
...
def smooth(x, y, name):
    ...
    data.to_excel(rf'E:\毕业论文\正文部分\{name}.xlsx')


if __name__ == '__main__':
    # 标点那边的比值要改一下
    # 要不要增加OOV比值？？？
    # distribution_two(param='add', name='add', scope=[0, 1], xlabel='词汇添加比例')
    # distribution_two(param='delete', name='delete', scope=[0, 1], xlabel='词汇删除比例')
    # distribution_two(param='reorder', name='reorder', scope=[0, 1], xlabel='词汇重排比例')
