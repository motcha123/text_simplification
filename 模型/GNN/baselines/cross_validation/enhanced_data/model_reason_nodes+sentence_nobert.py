...
class bert_gcn_trans(nn.Module):
    def __init__(self):
        ...
        # transformer模型
        self.transformer = nn.Transformer(d_model=768, num_encoder_layers=3, num_decoder_layers=3,
...
if __name__ == '__main__':
    ...
    with open(f'complex_sentences.pkl', mode='rb') as file:
        complex_sentences = pickle.load(file)
    with open(f'simple_sentences.pkl', mode='rb') as file:
        simple_sentences = pickle.load(file)

    # 序列化的AMR
    with open(f'amr_seq_list.pkl', mode='rb') as file:
        seq_amr = pickle.load(file)

    # 用于映射的矩阵
    with open(f'mat_list.pkl', mode='rb') as file:
        map_mats = pickle.load(file)

    # 用于GCN计算的边
    with open(f'edge_index_list.pkl', mode='rb') as file:
        edge_index_list = pickle.load(file)

    # 用于pad结点
    with open(f'nodes_pad.pkl', mode='rb') as file:
    ...
    class Mydataset(Dataset):
        ...
        def __getitem__(self, item):
            return (self.srcs_sentence[item], self.tgts[item], self.map_mats[item], self.srcs_amr_seq[item],
                    self.edge_index_list[item], self.amr_attention_mask[item], self.tgt_y[item], self.node_pad_list[item])
    ...
    model.load_state_dict(torch.load('node+sentence.pth'))
    ...
    with torch.no_grad():
        ...
        for idx, data in enumerate(dataloader):
            ...
            for i in range(511):
                ...
                input_tensor = tgts
                out = model(srcs_sentence, input_tensor, map_mats, srcs_amr_seq, edge_index_list, amr_attention_mask, node_pad_list)
