...
class bert_gcn_trans(nn.Module):
    ...
    def forward(self, src, tgt, map_mats, srcs_amr_seq, edge_index_list, amr_attention_mask, amr_seq):
        # 生成mask
        ...
        src_key_padding_mask = self.get_key_padding_mask(torch.concat([src, amr_attention_mask], dim=1))
        ...
        input_emb = torch.concat([src, srcs_amr_seq], dim=1)
...
if __name__ == '__main__':
    ...
    amr_seq = data_sentence_AMR_seq['input_ids']
    ...
    # model.load_state_dict(torch.load('model_B.pth'))
    ...
    # loss_path = 'loss_sentence+emb_gcn.txt'
    ...
    for i in range(300):
        ...
        for idx, data in enumerate(dataloader):
            srcs_sentence, tgts, map_mats, srcs_amr_seq, edge_index_list, amr_attention_mask, tgt_y, amr_seq = data
        ...
        torch.save(model.state_dict(), 'model_B.pth')  # 保存模型参数
        # print('模型保存！！！')
