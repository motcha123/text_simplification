...
if __name__ == '__main__':
    ...
    with open(complex_sentence_file_path, mode='rb') as complex_sentence_file:
        complex_sentences = pickle.load(complex_sentence_file)[:3872]

    with open(simple_sentence_file_path, mode='rb') as simple_sentence_file:
        simple_sentences = pickle.load(simple_sentence_file)[:3872]

    # 序列化的AMR
    with open('amr_seq_list.pkl', mode='rb') as file:
        seq_amr = pickle.load(file)[:3872]

    # 用于映射的矩阵
    with open('mat_list.pkl', mode='rb') as file:
        map_mats = pickle.load(file)[:3872]

    # 用于GCN计算的边
    with open('edge_index_list.pkl', mode='rb') as file:
        edge_index_list = pickle.load(file)[:3872]

    # 用于pad结点
    with open('node_pad_list.pkl', mode='rb') as file:
        node_pad_list = pickle.load(file)[:3872]
    ...
    # model.load_state_dict(torch.load('sentence+emb_gcn.pth'))
    ...
    for i in range(100):
        ...
        torch.save(model.state_dict(), 'sentence+emb_gcn.pth')  # 保存模型参数
        # print('模型保存！！！')
